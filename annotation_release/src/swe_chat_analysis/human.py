...
def main() -> None:
    ...
    prepare_parser.add_argument("--output-dir", default="outputs/human_relevant_v4_100_seed42")
    ...
    prepare_parser.add_argument("--max-chars", type=int, default=220000)
    prepare_parser.add_argument("--max-events", type=int, default=600)
    prepare_parser.set_defaults(language_filter=True, relevance_filter=True)
    prepare_parser.add_argument("--legacy", action="store_true", help="使用旧三阶段表单")
    serve_parser = sub.add_parser("serve")
    serve_parser.add_argument("--output-dir", default="outputs/human_relevant_v4_100_seed42")
    serve_parser.add_argument("--port", type=int, default=8765)
    export_parser = sub.add_parser("export")
    export_parser.add_argument("--output-dir", default="outputs/human_relevant_v4_100_seed42")
        ...
        if args.command == "export":
            ...
            if project.simple:
                ...
                with (destination / "analysis.csv").open("w", encoding="utf-8-sig", newline="") as file:
                    fields = ["session_id", "agent", "initial_coverage", "new_requirements", "requirement_source", "gap_driver", "instruction_quality", "literal_feasibility"] + list(human_simple.costs(next(iter(project.cases.values())), {"evolution": {"new_requirements": "没有"}}))
