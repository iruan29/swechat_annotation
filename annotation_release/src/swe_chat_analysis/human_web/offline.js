...
(() => {
  ...
  function summarize(rows) {
    const keys = ['effective_interactions','user_rounds','observed_tool_events','api_call_count','tool_call_count','total_tokens','duration_seconds'];
  ...
  window.offlineAPI = async (path, payload) => {
    ...
    if (url.pathname === '/api/case') {
      ...
      return {case_id:item.case_id,stage:'review',target_instruction_turn:item.target_instruction_turn,schema:copy(bundle.schema),
        ...
        interaction_quality:item.interaction_quality,simple:true,reading_stats:item.reading_stats,trace_scope:item.trace_scope};
    }
