...
VERSION = 'effective_interactions_v2'
...
def prompt_reason(text):
    ...
    if not text:
        return 'empty'
    if text.startswith(('Start a new change using the experimental artifact-driven approach.', 'Continue working on a change by creating the next artifact', 'Implement tasks from an OpenSpec change.', 'Archive a completed change in the experimental workflow.', 'Verify that implementation matches the change artifacts.', 'Explore mode. Think deeply.')):
