#!/usr/bin/env python3
"""Download SWE-Chat when needed, prepare packets, run Study 2, and summarize."""
...
def _data_ready(data_dir: Path) -> bool:
    return all(
        ...
        for path in (data_dir / "sessions.parquet", data_dir / "conversations.parquet")
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Reproducible SWE-Chat Study 2 pipeline: download, sample/process, "
            "judge with resume, and aggregate results"
        )
    )
    parser.add_argument("--data-dir", default="data/swe-chat")
    parser.add_argument("--output-dir", default="outputs/study2")
...
def main() -> None:
    ...
    needs_download = not _data_ready(data_dir) or transcripts_missing
    ...
    command = [
        sys.executable, "-m", "swe_chat_analysis.cli", "run-study2",
    ...
    if isinstance(pending, int) and pending > 0:
        raise SystemExit(
            f"Study 2 produced {completed}/{expected} valid annotations; "
            "rerun the same command to resume the remaining sessions"
        )
    print(f"Study 2 complete: {completed}/{expected}; summary={summary_path}", flush=True)
